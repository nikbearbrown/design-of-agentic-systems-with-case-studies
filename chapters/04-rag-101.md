...
**Deployment D — AlgoSensei, my own production system.**

I want to walk one more case, because it's the one I built. Composite cases are useful for covering the variable space. A real one is useful for showing the framework actually fires.

AlgoSensei is an adaptive DSA tutoring system I deployed for technical-interview preparation. The pedagogical constraint is unusual: the system must give *Socratic* hints — pointing toward the algorithm without naming it, writing functional code, or revealing the time-complexity insight that constitutes the solution. Any LLM, asked plainly, will violate that constraint.

- $C$ ≈ 49 chunks, 14 DSA patterns. Each chunk 300–400 tokens. Total corpus: roughly 18,000 tokens. Sources: CLRS, MIT OCW 6.006, Stanford CS161 lecture notes.
- $Q$ — variable. Production traffic from `adityaamitra-algosensei.hf.space`; I'll use a working figure of 200–500 hint requests/day during exam season.
- $U$ — near-zero. The DSA canon doesn't change quarter to quarter. I re-index when I add a new pattern, which is rare.
- $T$ — local. Every hint depends on one or two patterns. Two-Sum needs the hash-map chunk and possibly the brute-force-vs-optimized chunk. It does not need binary search or graph traversal.

Walk the flow. Rule 1 is interesting — 18K tokens fits in any context window, $U$ is nearly static. By the rule as stated, long-context wins. So why did I build RAG?

Because the rule omits a fifth variable I had to learn the hard way: the hint-leakage constraint isn't a retrieval problem; it's a *generation-time* constraint. RAG gave me an architectural property long-context couldn't: the retrieved chunks are formatted into a *structurally distinct region* of the prompt that the system prompt can reference by position — *"use only the conceptual scaffolding in [RETRIEVED]; do not paraphrase the algorithm name even if it appears in [RETRIEVED]"*. With long-context, the entire 18K-token corpus would sit in the same undifferentiated text body as the hint instruction, and a leakage gate over the full corpus is harder to specify than a leakage gate over four retrieved chunks. The decision wasn't between "RAG or long-context"; it was between "structured retrieval that the leakage gate can reason about" or "blob input where the gate has to scan everything."

The two-stage leakage gate — regex (fast, exact) followed by semantic similarity (slower, robust) — fires on the generated hint, not the retrieved context. Hints flagged by either stage trigger automatic regeneration up to 3×. Production telemetry across 110 synthetic test pairs measured **0% hint leakage**, **>90% retrieval recall at rank 4**, and **directional accuracy >85% across 14 patterns**.

**Verdict: RAG, for an unusual reason.** The corpus would have fit in long-context. The decision was driven by the architectural separation that retrieval gives the leakage gate, not by corpus size. This is the case I want you to remember when the four-variable framework gives a clean answer and the deployment still demands the other architecture: there's almost always a fifth variable the framework didn't name. Find yours before you commit.
...
RAG is a category the model can describe fluently and configure poorly.

Where the model genuinely helps: walking through the abstract pipeline with worked examples that vary corpus and query types.

Where the model does damage: recommending specific embedding models, chunk sizes, or rerankers without context about your corpus.

The rule: abstract pipeline from the model; concrete configuration from your data.
